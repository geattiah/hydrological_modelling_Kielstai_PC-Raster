############################################
# Model:Estimation of Evapration( Simulated Model for Open Fields)                                #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
#Proposed evaporation co-effcicient for Field
Field=scalar(0.5);
...
ETp = timeinputscalar(ETptimeseries, ezones)*Field;# Potential evaporation 
#Estimated Evapotranspiration(ET)
EET_Field.tss=timeoutput(pzones,ETp);
...
#Actual Evaporation for Field
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_Field.tss=timeoutput(pzones,ETa);#actual evaporation for field output .tss file

#Soil water content for Field
...
SWC_Field.tss=timeoutput(pzones,SWC); #output .tss for Soil water content for Field
...
Runoff_Field.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff for Field
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outlets for Field
report Q2_Field.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station for Field
report Q2_runoff_Field.tss=timeoutput(gauging,Q2);