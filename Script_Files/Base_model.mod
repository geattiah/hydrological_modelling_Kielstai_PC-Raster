############################################
# Model:Simulated Base Model derived from the Digital Spatial Analysis Class                                  #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
ETp = timeinputscalar(ETptimeseries, ezones);# Potential evaporation 
#Estimated Evapotranspiration(ET)
EET_Base.tss=timeoutput(pzones, ETp);

day = if (day >365 then 0 else day + 1);
#Calculating interception
interception_content = min(Precipitation, Inter_cap); 
interception = min(ETp, interception_content);#Inteception at base
...
#Actual Evaporation at base model
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_Base.tss=timeoutput(pzones,ETa);#actual evaporation at base output .tss file

#Soil water content at base model
...
SWC_Base.tss=timeoutput(pzones,SWC); #output .tss for Soil water content at base
...
# Runoff at base model
Runoff = runoff_surface + gw_outflow;	# runoff is from both surface runoff and ground water outflow
Runoff_base.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff at base
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outlets at base
report Q2_Base.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station at base
report Q2_runoff_Base.tss=timeoutput(gauging,Q2);