############################################
# Model:Estimation of Evapration( Simulated Model for Landuse)                                #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
dynamic
#coefficient for landuse evaporation = landuse map
Precipitation = timeinputscalar(Raintimeseries, pzones);# Measured values for precipitation
ETp = timeinputscalar(ETptimeseries, ezones)*Landuse;# Potential evaporation 
#Estimated Evapotranspiration(ET)
EET_Landuse.tss=timeoutput(pzones, ETp);
...
#Actual Evaporation for Landuse
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_Landuse.tss=timeoutput(pzones,ETa);#actual evaporation for Landuse output .tss file

#Soil water content for Landuse
...
SWC_Landuse.tss=timeoutput(pzones,SWC); #output .tss for Soil water content for Landuse
...
Runoff_Landuse.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff for Landuse
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outlets for Landuse
report Q2_Landuse.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station for Landuse
report Q2_runoff_Landuse.tss=timeoutput(gauging,Q2);