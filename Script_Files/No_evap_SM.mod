############################################
# Model:Estimation of Evapration( Simulated Model with no Evaporation)                                #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
#Proposed evaporation co-effcicient for no evaporation
No_evap=scalar(0);
...
ETp = timeinputscalar(ETptimeseries, ezones)*No_evap;# Potential evaporation 
#Estimated Evapotranspiration(ET)
EET_No_evap.tss=timeoutput(pzones, ETp);
...
rest_et = ETp - interception;# rest of evaporation calsulated by potential evaporation minus intercepted precipitation
...
#Actual Evaporation with no potential evaporation
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_No_evap.tss=timeoutput(pzones,ETa);#actual evaporation .tss file

#Soil water content with no potential evaporation
...
SWC_No_evap.tss=timeoutput(pzones,SWC); #output .tss for Soil water content no potential evaporation
...
Runoff_No_evap.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff no potential evaporation
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outlets
report Q2_No_evap.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station 
report Q2_runoff_No_evap.tss=timeoutput(gauging,Q2);