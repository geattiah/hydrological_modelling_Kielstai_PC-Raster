############################################
# Model:Estimation of Evapration( Simulated Model for Shrub)                                #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
#Proposed evaporation co-effcicient for Shrub
Shrub=scalar(1.4);
...
ETp = timeinputscalar(ETptimeseries, ezones)*Shrub;# Potential evaporation 
#Estimated Evapotranspiration(ET)
EET_Shrub.tss=timeoutput(pzones, ETp);
...
#Actual Evaporation for Shrub
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_Shrub.tss=timeoutput(pzones,ETa);#actual evaporation for Shrub output .tss file

#Soil water content for Shrub
...
SWC_Shrub.tss=timeoutput(pzones,SWC); #output .tss for Soil water content for Shrub
...
Runoff_Shrub.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff for Shrub
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outlets for Shrub
report Q2_Shrub.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station for Shrub
report Q2_runoff_Shrub.tss=timeoutput(gauging,Q2);