############################################
# Model:Estimation of Evapration( Simulated Model for Grass)                                #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
#Proposed evaporation co-effcicient for Grass
Grass=scalar(1.6);
...
ETp = timeinputscalar(ETptimeseries, ezones)*Grass;# Potential evaporation
#Estimated Evapotranspiration(ET) 
EET_Grass.tss=timeoutput(pzones, ETp);
...
#Actual Evaporation for Grass
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_Grass.tss=timeoutput(pzones,ETa);#actual evaporation for Grass output .tss file

#Soil water content for Grass
...
SWC_Grass.tss=timeoutput(pzones,SWC); #output .tss for Soil water content for Grass
...
Runoff_Grass.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff for Grass
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outletsfor Grass
report Q2_Grass.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station for Grass
report Q2_runoff_Grass.tss=timeoutput(gauging,Q2);