############################################
# Model:Estimation of Evapration( Simulated Model for Forest)                                #
# Date:15th July, 2017                                    #
# Version: 1.0                             #
# Author: Gifty Attiah 
...
#Proposed evaporation co-effcicient for Forest
Forest=scalar(1.8);
...
ETp = timeinputscalar(ETptimeseries, ezones)*Forest;# Potential evaporation 
#Estimated Evapotranspiration(ET)
EET_Forest= timeoutput(pzones, ETp);
...
#Actual Evaporation for forest
ETa = if (SWC > soil_start_reduction then rest_et else rest_et*(SWC/soil_start_reduction));	# actual evapotranpiration
ETa_Forest.tss=timeoutput(pzones,ETa);#actual evaporation for forest output .tss file

#Soil water content for Forest
...
SWC_Forest.tss=timeoutput(pzones,SWC); #output .tss for Soil water content for forest
...
Runoff_Forest.tss=timeoutput(gauging,Runoff);#output .tss for initial runoff at base
...
Q2_sum = Q2_sum + Q2;
# Output .tss kinematic runoff for all outletsfor forest
report Q2_Forest.tss=timeoutput(outlet,Q2);
## Output .tss kinematic runoff for virtual gauging station for forest
report Q2_runoff_Forest.tss=timeoutput(gauging,Q2);